...
def add_engineered_features(df: pd.DataFrame, frequency_lookup: dict[str, dict[str, int]]) -> pd.DataFrame:
    ...
    out["log_khoang_cach"] = np.log1p(pd.to_numeric(out["khoang_cach_den_duong_chinh_m"], errors="coerce").clip(lower=0))
    out["log_do_rong_ngo"] = np.log1p(pd.to_numeric(out["do_rong_ngo_nho_nhat_m"], errors="coerce").clip(lower=0))
...
def build_hybrid_embedding(
...
) -> tuple[pd.DataFrame, KMeans, list[str]]:
    train = train_df.copy()
    ...
    cluster_model = KMeans(n_clusters=n_clusters, random_state=config.random_state, n_init=20)
    clusters = cluster_model.fit_predict(embedding)
    distances = cluster_model.transform(embedding)
    assigned_distance = distances[np.arange(len(train)), clusters]
    ...
    return train, cluster_model, spectral_cols
...
def build_location_fallback_templates(segment_embedding_df: pd.DataFrame, spectral_cols: list[str]) -> dict[str, Any]:
    fallback = {
        "global": {},
        "phuong": {},
        "quan": {},
    ...
    for level in ["phuong_norm", "quan_norm"]:
        grouped = (
            segment_embedding_df.groupby(level)[spectral_cols + ["cluster_confidence"]]
            .mean()
            .reset_index()
        )
        cluster_mode = (
...
def assign_inference_representation(
    ...
    for row in out.itertuples(index=False):
        ...
        if segment_info and segment_info["train_sample_count"] > 0:
            ...
            assigned_cluster.append(int(segment_info["cluster"]))
            cluster_conf.append(float(segment_info["cluster_confidence"]))
            for column in spectral_cols:
        ...
        if row.has_valid_coordinate and not train_segments_with_geo.empty:
            ...
            for column in spectral_cols:
                assigned_spectral[column].append(float(np.average(selected[column], weights=weights)))
            cluster_weights: dict[int, float] = {}
            for cluster_value, weight in zip(selected["cluster"], weights):
                cluster_weights[int(cluster_value)] = cluster_weights.get(int(cluster_value), 0.0) + float(weight)
            best_cluster = max(cluster_weights.items(), key=lambda item: item[1])[0]
            assigned_cluster.append(best_cluster)
            cluster_conf.append(float(cluster_weights[best_cluster] / weight_sum))
        ...
        for column in spectral_cols:
            assigned_spectral[column].append(float(template.get(column, 0.0)))
        assigned_cluster.append(int(template["cluster"]))
        cluster_conf.append(float(template["cluster_confidence"]) * 0.5)
...
def build_comparable_features(
    ...
    for query_pos, query_row in queries.iterrows():
        ...
        if len(same_purpose) >= config.min_comparable_count:
            candidates = same_purpose
        same_type = candidates[candidates["phan_loai_kho"] == query_row["phan_loai_kho"]].copy()
        if len(same_type) >= config.min_comparable_count:
            candidates = same_type

        area = float(query_row["dien_tich_m2"])
        area_30 = candidates[candidates["dien_tich_m2"].between(area * 0.7, area * 1.3, inclusive="both")].copy()
        if len(area_30) >= 3:
            candidates = area_30
        else:
        ...
        distance_penalty = np.where(np.isfinite(candidates["distance_m"]), np.clip(candidates["distance_m"] / 2000, 0, 1), 1.0)
        area_penalty = np.clip(np.abs(np.log((candidates["dien_tich_m2"] + 1e-6) / max(area, 1e-6))), 0, 1)
        candidates["similarity_score"] = (
            0.35 * candidates["spatial_similarity"]
            + 0.25 * candidates["feature_similarity"]
            + 0.15 * candidates["access_similarity"]
            + 0.15 * candidates["location_similarity"]
            + 0.10 * candidates["same_cluster"]
            - 0.05 * distance_penalty
            - 0.05 * area_penalty
        )

        topk = candidates.sort_values(["similarity_score", "distance_m"], ascending=[False, True]).head(
            config.top_k_comparable
        )
        ...
        if topk.empty:
            ...
            p25 = float(topk["tong_gia"].quantile(0.25))
            median = float(topk["tong_gia"].median())
            p75 = float(topk["tong_gia"].quantile(0.75))
            same_cluster_ratio = float(topk["same_cluster"].mean())
            score_strength = float(topk["similarity_score"].clip(lower=0).mean())
            count_score = min(1.0, len(topk) / config.top_k_comparable)
...
def model_feature_layout(
    ...
    categorical = ["phan_loai_kho", "muc_dich_su_dung_dat"]
    return {
        ...
        "sets": {
            ...
            + POSITION_FEATURE_COLS
            + spectral_cols
            + ["assigned_cluster", "cluster_confidence"],
...
def build_oof_predictions(
    ...
    for fold_id, (fit_idx, val_idx) in enumerate(folds.split(clean_train)):
        ...
        fold_output = fold_val[["row_id", "segment_key", "pho_canonical", "nam"]].copy()
...
def train_pipeline(config: PipelineConfig, phase_logger: PhaseLogger) -> dict[str, Any]:
    ...
    train, cluster_model, spectral_cols = build_hybrid_embedding(
        train,
    ...
    train_oof_predictions = add_error_columns(train_oof_predictions)
    phase_logger.add(
...
def inference_pipeline(config: PipelineConfig, phase_logger: PhaseLogger) -> dict[str, Any]:
    ...
    fallback_templates = build_location_fallback_templates(artifacts["segment_embedding"], artifacts["spectral_cols"])
